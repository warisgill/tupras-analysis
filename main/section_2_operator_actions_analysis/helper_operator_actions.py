...
def constructSingleAlarmsActionsGraph(df_alarms, df_actions):  # case 3
    ...
    # ---------- Adding Nodes----------------
    _ = list(filter(partial(addOrUpdateNode, g, False),
                    df_alarms["SourceName"]))
    _ = list(filter(partial(addOrUpdateNode, g, True),
    ...
    edges = [("Operator->"+action["SourceName"], alarm["SourceName"])
             for action in actions for alarm in alarms if checkAction_OnAlarm(action, alarm)]
...
def getFinalOperatorAlarmRelationGraph(df_alarms, df_actions, num_graphs, min_graphs_intersection_filter):
    ...
    graphs = constructMultipleAlarmsActionsGraphs(
